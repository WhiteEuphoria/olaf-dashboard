            ...
            <div class="auth">
                ...
                <div class="loading">
                    <div class="loading__circle">
                        ...
                        <span class="loading__percent">{{ $isApproved ? '100%' : '56%' }}</span>
                    </div>
                </div>

                <div class="verify-text">
                    <p style="font-weight: 600; font-size:1.15rem; margin-bottom:0.75rem;">{{ $title }}</p>
                    <p>{!! nl2br(e($message)) !!}</p>
                    ...
                    @if ($isApproved)
                        <a class="btn" href="{{ route('user.dashboard') }}" style="margin-top:1.5rem; display:inline-flex;">Перейти в кабинет</a>
                    @else
                        <form method="POST" action="{{ route('user.logout') }}" style="margin-top:1.5rem;">
                            @csrf
